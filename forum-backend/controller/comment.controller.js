...
module.exports = {
  ...
  getSingleComment: async (req, res) => {
    try {
      ...
      await commentModel.populate(result, {
        path: "commenter",
        select: "-password",
      });
      res.json({ result });
  ...
  editComment: async (decoded, params, editId) => {
    ...
    const editedComment = await commentModel.findByIdAndUpdate(commentId, {
      content: content,
    });
  ...
  removeComment: async (commentId) => {
    const result = await commentModel.findByIdAndUpdate(commentId.id, {
      content: "deleted",
    });
    if (!result) {
      return "No comment exists by that ID";
