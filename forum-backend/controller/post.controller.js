...
module.exports = {
  ...
  createNewPost: async (userId, postBody, params) => {
    const { title, content, img } = postBody;
  ...
  editPost: async (content, params) => {
    ...
    const post = await postModel.findByIdAndUpdate(postId, {
      title: content[0],
      textSubmission: content[1],
      imageSubmission: content[2],
    });
    return post;
  },
  ...
  updateUpvote: async (decoded, params) => {
    ...
    if (downvoted.includes(user)) {
      ...
      const removedDownvote = await postModel
        .findByIdAndUpdate(postId, { downvotedBy: upvoted })
        .exec();
    }
    ...
    if (upvoted.includes(user)) {
      ...
      const removedUpvote = await postModel
        .findByIdAndUpdate(postId, { upvotedBy: upvoted })
        .exec();
    ...
    if (!upvoted.includes(user)) {
      ...
      const addUpvote = await postModel
        .findByIdAndUpdate(postId, { upvotedBy: upvoted })
        .exec();
      return addUpvote;
    }
  ...
  updateDownvote: async (decoded, params) => {
    ...
    if (upvoted.includes(user)) {
      ...
      const removeUpVote = await postModel
        .findByIdAndUpdate(postId, { upvotedBy: upvoted })
        .exec();
    }
    ...
    if (downvoted.includes(user)) {
      ...
      const removedDownvote = await postModel
        .findByIdAndUpdate(postId, { downvotedBy: downvoted })
        .exec();
